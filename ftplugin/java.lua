...
local function runInActiveTerminal()
  ...
  if (terminal_window ~= nil) then
    ...
    vim.cmd("startinsert")
    vim.api.nvim_feedkeys([[javac "]] .. vim.fs.dirname(filePath) .. [[/Main.java" && java Main]] .. "\n", "", "")

  else
    --apro nuovo terminale e inserisco il comando in base alla modalità
    vim.cmd([[vsplit | terminal]] .. "\n")
    vim.cmd("startinsert")
    vim.api.nvim_feedkeys([[javac "]] .. vim.fs.dirname(filePath) .. [[" && java Main]] .. "\n", "", "")
...
vim.api.nvim_buf_set_keymap(0, 'n', '<space>r', '', { noremap = true, callback = function() return runInActiveTerminal("single") end})
vim.api.nvim_buf_set_keymap(0, 'n', '<space>R', '', { noremap = true, callback = function() return terminateExecution() end })
